...
export default function LoginAdminForm() {
    ...
    return (
        <>
            {!isOpen && <Button onClick={onOpen}>Sign in</Button>}
            <Modal isOpen={isOpen} onClose={onClose}>
                <ModalOverlay/>
                <ModalContent maxW="400px" m="2em auto" border='1px solid gray' borderRadius="5px" p="1em" pt="2em">
                    <ModalHeader display="flex" justifyContent="flex-end" p="0">
                        <ModalCloseButton bg="none" w="fit-content" h="fit-content" p="0"/>
                    </ModalHeader>
                    <ModalBody mt="1em">
                        <form onSubmit={handleSubmit(handleFormSubmit)}>
                            <Text fontSize='xl'>Sign in to your account</Text>
                            <FormControl w="100%" mt="1em">
                                <FormLabel htmlFor="username" mb="0">Email address</FormLabel>
                                <Input w="100%" h="30px" type='email' id="username"  {...register('username', {required: true})}/>
                            </FormControl>
                            <FormControl w="100%" mt="1em">
                                <FormLabel htmlFor="password" mb="0">Password</FormLabel>
                                <Input w="100%" type='password' id="password" h="30px"  {...register('password', {required: true})}/>
                            </FormControl>
